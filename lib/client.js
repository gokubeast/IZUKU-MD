                    ...
                    try {
                        ...
                        if (citel.text && !citel.isGroup) {
                            if (zx < 25) {
                                ...
                                var textuser = budy
                                let fetchtext = await fetchk(`http://api.brainshop.ai/get?bid=176187&key=fpP88ip9qevEFyCz&uid=[${diffuser}]&msg=[${textuser}]`);
                            ...
                            if (zx < 20) {
                                var diffuser = citel.sender.split("@")[0];
                                let fetchk = require("node-fetch");
                                let fetchtext = await fetchk(`http://api.brainshop.ai/get?bid=176187&key=fpP88ip9qevEFyCz&uid=[${diffuser}]&msg=[${citel.text}]`);
                                let json = await fetchtext.json();
                                let { cnt } = json;
				    console.log(cnt)
                                citel.reply(cnt);
